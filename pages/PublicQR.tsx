...
const PublicQR: React.FC = () => {
  ...
  const customer = db.getAll<Customer>('customers').find(c => c.qrToken === token);
  const commerce = customer ? db.getById<Commerce>('commerces', customer.commerceId) : null;
    ...
    <div className="min-h-screen bg-slate-50 p-4 pb-12 flex flex-col items-center">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center pt-8 space-y-3">
          {commerce?.logoUrl ? (
            <img src={commerce.logoUrl} alt="Logo" className="w-20 h-20 rounded-[24px] mx-auto border shadow-sm object-cover" />
          ) : (
          ...
          <div>
            <h2 className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Club de Beneficios</h2>
                ...
                return (
                  <div key={reward.id} className="bg-white p-5 rounded-[24px] shadow-sm border border-slate-100">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`text-[10px] font-black px-2 py-1 rounded-full uppercase ${isUnlocked ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'}`}>
                        {reward.pointsThreshold} pts
                      </span>
                    </div>
                    <div className="w-full h-2.5 bg-slate-50 rounded-full overflow-hidden border border-slate-100">
                      <div 
                        className={`h-full transition-all duration-1000 ${isUnlocked ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.3)]' : 'bg-blue-600'}`}
