...
const SLOTS_MERCHANT = [
  ...
  { id: 'beneficios',     label: 'Beneficios',     Icon: Sparkles,        view: 'commerce-settings', tab: 'beneficios' },
...
export default function BottomNavV2({
  ...
  const isActive = (slot) => {
    ...
    if (slot.id === 'beneficios') {
      return ['beneficios', 'premios', 'promociones'].includes(currentTab)
    }
      ...
      >
            ...
            >
              ...
              <span style={{ position: 'relative', display: 'inline-flex' }}>
                <slot.Icon
                  ...
                  color={isActive(slot) ? ACTIVE : INACTIVE}
                  style={isActive(slot) ? { filter: 'drop-shadow(0 0 6px rgba(189,75,248,0.6))' } : undefined}
                />
        ...
        <button
          ...
          style={{
            ...
            boxShadow: '0 8px 24px rgba(189,75,248,0.45), 0 4px 12px rgba(254,80,0,0.3)',
            cursor: 'pointer',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
