...
export default function PrizeWizardModal({
  ...
  const unitColor = commerce?.prog_type === 'stars' ? '#8B5CF6' : '#EC4899'
  const UnitIcon  = commerce?.prog_type === 'stars' ? Star : Coins

  const STEP_HEROES = [
    null,
    { title: '¿Qué premio vas a ofrecer?', subtitle: 'Pensá en algo que entusiasme a tus clientes' },
    { title: '¿Cuánto cuesta?',            subtitle: `En cuántas ${unitLabel} se canjea` },
    { title: 'Sumá una foto',              subtitle: 'Opcional. Las fotos hacen tu premio más atractivo' },
  ]
  ...
  const enabled  = stepValid(step) && !addingPrize && !uploadingImg && (!isLast || !isEditing || hasChanges)
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '14px 18px 8px', flexShrink: 0, position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div style={{ fontFamily: FN, fontSize: 11, fontWeight: 700, color: C.dust, letterSpacing: '.08em', textTransform: 'uppercase' }}>
            {isEditing ? 'Editando' : 'Nuevo premio'} · Paso {step} de {TOTAL_STEPS}
          </div>
      ...
      {/* Hero del paso (sin icono cuadrado para que se sienta más liviano que el alta de negocio) */}
      <div style={{ padding: '20px 22px 6px', flexShrink: 0, position: 'relative', zIndex: 1, textAlign: 'center' }}>
            ...
            <div style={{ maxWidth: 440, margin: '0 auto' }}>
              ...
              {newPrize.img_url ? (
                ...
                <label htmlFor="prize-wizard-img-input" style={{
                  ...
                  border: '2px dashed rgba(189,75,248,0.32)',
                  borderRadius: 18,
                  padding: '36px 20px',
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  background: newPrize.img_url ? `url(${newPrize.img_url}) center/cover` : 'rgba(189,75,248,0.18)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  flexShrink: 0,
                }}>
          ...
          {addingPrize && isLast
            ...
            : (isLast
                ? (isEditing
                    ...
                    : 'Crear premio')
                : 'Continuar')}
          {!addingPrize && !isLast && <ChevronRight size={16} strokeWidth={2.4} />}
        </button>
      </div>
    </div>
  )
