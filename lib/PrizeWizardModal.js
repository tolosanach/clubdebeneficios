        ...
        {isEditing ? (
          <div style={{ position: 'relative', display: 'inline-block', marginBottom: 12 }}>
            <div style={{
              ...
              background: newPrize.img_url ? `url(${newPrize.img_url}) center/cover` : 'rgba(113,49,225,0.14)',
              border: '1px solid rgba(113,49,225,0.40)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: '0 8px 24px -8px rgba(113,49,225,0.55)',
            }}>
              {!newPrize.img_url && <Tag size={26} color={C.v} strokeWidth={2.2} />}
            </div>
            {/* Lapicito para cambiar la foto — reusa el file input del paso foto */}
            <label htmlFor="prize-wizard-img-input" title="Cambiar foto" aria-label="Cambiar foto"
              style={{ position: 'absolute', bottom: -6, right: -6, width: 28, height: 28, borderRadius: '50%', background: '#6F30DF', border: '2px solid #0a0812', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.45)' }}>
              <Pen size={13} color="#fff" strokeWidth={2.4} />
            </label>
            {newPrize.img_url && (
              <button type="button" onClick={() => setNewPrize(p => ({ ...p, img_url: '' }))} title="Quitar foto" aria-label="Quitar foto"
                style={{ position: 'absolute', top: -6, right: -6, width: 24, height: 24, borderRadius: '50%', background: '#f87444', border: '2px solid #0a0812', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 0 }}>
                <Trash2 size={11} color="#fff" strokeWidth={2.4} />
              </button>
            )}
          </div>
        ) : heroData?.Icon && (
