...
// Un PREMIO es algo concreto que se entrega o regala a cambio del balance
// de estrellas/puntos del cliente. Puede ser:
//   • Un PRODUCTO físico → café, libro, ramo, herramienta, prenda, etc.
//   • Un SERVICIO regalado → corte de pelo, sesión, lavado, baño de
//     mascota, evaluación, asesoramiento, retoque, masaje, etc.
//
// NO es premio:
//   • Descuentos %OFF (10% OFF, 20% OFF en próxima compra, etc.) — esos
//     son `discount_next` y viven aparte como BENEFICIOS automáticos.
//     NO entran en el catálogo de premios canjeables del cliente.
//   • Promos como "días con suma ×2" — esos son `double_points`, también
//     beneficios separados.
//
// Si tenés que decidir si un item es premio o no, hacete esta pregunta:
//   "¿Es algo que el comerciante ENTREGA al cliente cuando canjea su
//    balance, o es una rebaja sobre el precio de algo que le va a vender?"
// Si entrega → premio. Si rebaja → descuento (no va acá).
// ───────────────────────────────────────────────────────────────────────
//
// Se usa en:
...
/**
 * Devuelve premios sugeridos paraun comercio segun sus categorias.
