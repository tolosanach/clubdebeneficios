...
/**
 * Valida si VAPID está configurado en el servidor (sin traer la key entera).
 * Devuelve { vapid_configured: bool, vapid_public_key: string|null }.
 */
export async function checkVapidConfig() {
  try {
    ...
    return { vapid_configured: false, vapid_public_key: null }
  }
}
...
export function attachServiceWorkerMessageListener() {
  ...
  function handler(event) {
    if (event.data?.type === 'clufix:notification') {
