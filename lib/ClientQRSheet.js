...
import { QrCode, ScanLine, X, ArrowLeft, Share2, User } from 'lucide-react'
...
const VIOLET = '#D6198C' // fucsia del cliente (diferencia visual vs negocio)
...
export default function ClientQRSheet({ open, onClose, profile }) {
  ...
  async function buildShareImage() {
    ...
    ctx.textAlign = 'center'
    ctx.textBaseline = 'top'
    ctx.fillText('ESCANEÁ ESTE QR EN CAJA', W / 2, INSTR_Y)
    ...
    return (
      <div role="dialog" aria-modal="true" style={{
        ...
        <div style={{
          ...
          <img src="/clufix_logo.svg" alt="Clufix" style={{ height: 60 }} />
        </div>

        {/* Badge de modo — fucsia + ícono para leer de un vistazo que es el pase del cliente */}
        <div style={{
          display: 'inline-flex', alignItems: 'center', gap: 8,
          padding: '7px 16px 7px 13px', marginBottom: 16,
          background: 'rgba(0,0,0,0.20)', borderRadius: 99,
        }}>
          <User size={16} strokeWidth={2.4} color="#fff" />
          <span style={{
        ...
        {/* Nombre — kicker + nombre en texto (no estilo botón) */}
        {displayName && (
