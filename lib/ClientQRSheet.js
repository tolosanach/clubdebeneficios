...
export default function ClientQRSheet({ open, onClose, profile }) {
  ...
  async function handleShare() {
    ...
    const fileName = 'mi-qr-benefix.png'
    const file = blob ? new File([blob], fileName, { type: 'image/png' }) : null

    if (file && typeof navigator !== 'undefined' && navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({ files: [file], title: 'Mi QR de Benefix' })
        return
      } catch {}
    }
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({ title: 'Mi QR de Benefix' })
        return
      } catch {}
    }
    if (blob) {
    ...
    return (
      <div role="dialog" aria-modal="true" style={{
        ...
        <div style={{
          ...
          <img src="/brand/logo-benefix-wordmark-white.svg" alt="Benefix" style={{ height: 60 }} />
        </div>
