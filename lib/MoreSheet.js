...
export default function MoreSheet({ open, onClose, onNavigate, onLogout, profile }) {
  useEffect(() => {
    if (!open) return
  ...
  const items = [
    ...
    { label: 'Reportes y segmentación', Icon: PieChart, action: navAndClose('commerce-settings', 'reportes') },
    { label: 'Historial',               Icon: Clock,    action: navAndClose('commerce-settings', 'historial') },
    { label: 'Automatizaciones',        Icon: Bot,      action: navAndClose('commerce-settings', 'automatizaciones'), badge: 'PRO' },
  ]
  const accountItems = [
    { label: 'Cuenta',                  Icon: User,     action: navAndClose('commerce-settings', 'cuenta') },
