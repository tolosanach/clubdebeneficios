  ...
  return (
    <nav style={{ position:'sticky', top:0, zIndex:100, height:68, background:'#fff', borderBottom:'1px solid #f0eaff', display:'flex', alignItems:'center', padding:'0 clamp(16px,5vw,48px)', gap:32, fontFamily:FN }}>
      ...
      <div style={{ display:'flex', gap:10 }}>
        <Link href="/" style={{ color:DK, padding:'0.5rem 1rem', borderRadius:100, border:`1.5px solid ${DK}`, fontFamily:FN, fontWeight:500, fontSize:'0.9rem', textDecoration:'none', display:'inline-block' }}>
          Iniciar sesión
        </Link>
        <Link href="/" style={{ background:DK, color:LT, padding:'0.5rem 1.25rem', borderRadius:100, fontFamily:FN, fontWeight:700, fontSize:'0.9rem', textDecoration:'none', display:'inline-block' }}>
          Registrarse
        </Link>
      </div>
    </nav>
...
const PLANS = [
  { name:'Free', price:'$0', period:'/mes', popular:false, features:[
    ...
    { label:'WhatsApp semi-automático', ok:false }, { label:'Reactivar inactivos', ok:false },
  ]},
  ...
  { name:'Pro', price:'$45.000', period:'/mes', popular:false, features:[
    ...
    { label:'QR del local', ok:true }, { label:'Directorio público', ok:true },
    { label:'Cupones de descuento', ok:true }, { label:'Días bonus ×2', ok:true },
    { label:'WhatsApp semi-automático', ok:true }, { label:'Reactivar inactivos', ok:true },
  ]},
]

// ─── Benefit icons ────────────────────────────────────────────────────────────
const BENEFIT_ICONS = {
  star:           <IconStar size={22} stroke={1.5} color={DK} />,
  ticket:         <IconTicket size={22} stroke={1.5} color={DK} />,
  bolt:           <IconBolt size={22} stroke={1.5} color={DK} />,
  'message-circle': <IconMessageCircle size={22} stroke={1.5} color={DK} />,
  coin:           <IconCoin size={22} stroke={1.5} color={DK} />,
  calendar:       <IconCalendar size={22} stroke={1.5} color={DK} />,
  pill:           <IconPill size={22} stroke={1.5} color={DK} />,
}

// ─── App benefits ─────────────────────────────────────────────────────────────
const APP_FEATURES = [
  { icon:<IconQrcode size={24} stroke={1.5} color={DK} />,  title:'QR instantáneo',        desc:'Tu club tiene un QR único. Lo imprimís y lo ponés en el mostrador.' },
  { icon:<IconUsers size={24} stroke={1.5} color={DK} />,   title:'Panel de clientes',      desc:'Ves quién vino, cuándo y cuánto acumula cada uno.' },
  { icon:<IconStar size={24} stroke={1.5} color={DK} />,    title:'Estrellas o puntos',     desc:'Elegís el sistema que mejor funciona para tu negocio.' },
  { icon:<IconBell size={24} stroke={1.5} color={DK} />,    title:'Notificaciones push',    desc:'Tus clientes reciben avisos cuando canjearon o acumularon.' },
  { icon:<IconRobot size={24} stroke={1.5} color={DK} />,   title:'Soporte con IA',         desc:'Asistente disponible 24/7 dentro de la app.' },
  { icon:<IconMapPin size={24} stroke={1.5} color={DK} />,  title:'Directorio público',     desc:'Tu local aparece en el directorio de Clufix para que te descubran.' },
]

// ─── Rubro icons ──────────────────────────────────────────────────────────────
const RUBRO_ICON_MAP = {
  scissors:          <IconScissors size={28} stroke={1.5} color={LT} />,
  pill:              <IconPill size={28} stroke={1.5} color={LT} />,
  hanger:            <IconHanger size={28} stroke={1.5} color={LT} />,
  'tools-kitchen-2': <IconToolsKitchen2 size={28} stroke={1.5} color={LT} />,
  barbell:           <IconBarbell size={28} stroke={1.5} color={LT} />,
  'building-store':  <IconBuildingStore size={28} stroke={1.5} color={LT} />,
}
      ...
      <section style={{ background:V1, padding:`80px ${px}`, textAlign:'center', color:'#fff' }}>
        <FadeIn>
          ...
          <div style={{ display:'flex', gap:14, justifyContent:'center', flexWrap:'wrap' }}>
            <Link href="/" style={btnDark}>Crear mi club gratis</Link>
        ...
        <FadeIn style={{ display:'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap:48, maxWidth:960, margin:'0 auto', alignItems:'center' }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:20, padding:'28px 24px' }}>
            ...
            <div style={{ borderTop:'1px solid rgba(255,255,255,0.08)', paddingTop:16, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              <div style={{ background:LT, color:DK, borderRadius:100, padding:'4px 12px', fontSize:'0.75rem', fontWeight:700, fontFamily:FN }}>Sumarme</div>
            </div>
          </div>
        </FadeIn>
      </section>
          ...
          {TESTIMONIOS_GENERIC.map((t, i) => (
            <FadeIn key={i} delay={i * 80}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:38, height:38, borderRadius:'50%', background:LT, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, color:DK, fontSize:'0.9rem', flexShrink:0 }}>
                  {t.initials}
                </div>
                <div>
              ...
              <div style={{ borderTop:'1px solid #f0eaff', flex:1 }}>
                {plan.features.map(f => (
                  <div key={f.label} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid #f9f6ff' }}>
                    {f.ok ? <IconCheck size={16} stroke={2.5} color={DK} /> : <IconX size={16} stroke={2.5} color='rgba(34,0,51,0.2)' />}
                    <span style={{ fontSize:'0.88rem', color: f.ok ? V1 : 'rgba(26,0,80,0.35)', fontFamily:FN }}>{f.label}</span>
                  </div>
                ))}
              </div>
              <Link href="/" style={{ ...btnLight, width:'100%', textAlign:'center', marginTop:20, display:'block', boxSizing:'border-box' }}>
                Empezar
              </Link>
            </FadeIn>
          ))}
