...
function Nav({ onLoginClick, onLogoClick, onGoPanel, user }) {
  ...
  return (
    <nav style={{
      ...
      <button onClick={onLogoClick} style={{ background:'none', border:'none', padding:0, cursor:'pointer', flexShrink:0, display:'flex', alignItems:'center' }}>
        <img src="/clufix_logo.svg" alt="Clufix" style={{ height:'44px', width:'auto', display:'block' }} />
      </button>
      {user
        ? <button onClick={onGoPanel} style={{ height:'44px', background:LT, color:DK, padding:'0 1.5rem', borderRadius:100, border:'none', cursor:'pointer', fontFamily:FN, fontWeight:700, fontSize:'0.9rem' }}>
            Ir a mi panel
          </button>
...
function Footer() {
  return (
    <footer style={{ background:DK, borderTop:'1px solid rgba(237,213,246,0.12)', padding:'1.5rem 2rem', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'1rem', fontFamily:FN }}>
      <img src="/clufix_logo.svg" alt="Clufix" height={28} style={{ display:'block' }} />
      <span style={{ fontSize:'0.78rem', color:'rgba(237,213,246,0.45)' }}>© 2026 · clufix.com.ar · Todos los derechos reservados</span>
    </footer>
  )
}
    ...
    <div style={{ fontFamily:FN, overflowX:'hidden', isolation:'isolate' }}>
      ...
      <section style={{ background:'#220033', padding:`80px ${px}`, textAlign:'center' }}>
        <FadeIn>
          ...
          <div style={{ overflow:'hidden', width:'100%', marginTop:40, WebkitMaskImage:'linear-gradient(to right, transparent, #000 12%, #000 88%, transparent)', maskImage:'linear-gradient(to right, transparent, #000 12%, #000 88%, transparent)' }}>
            <div style={{ display:'flex', width:'max-content', animation:'homepub-marquee 20s linear infinite', willChange:'transform' }}>
              {[0,1,2,3].map(i => (
                <span key={i} style={{ display:'flex', alignItems:'center', gap:20, paddingRight:56, flexShrink:0, fontSize:'0.78rem', color:'rgba(255,255,255,0.35)', letterSpacing:'3px', textTransform:'uppercase', fontFamily:FN, whiteSpace:'nowrap' }}>
                  <span>ESCANEÁ</span>
      ...
      {/* ── Rubros (oscuro) ─────────────────────────────────────────────── */}
      <section id="rubros" style={{ background:DK, padding:`${sectionPy} ${px}`, textAlign:'center', minHeight:'100svh', display:'flex', flexDirection:'column', justifyContent:'center' }}>
      ...
      {/* ── Fundadores (oscuro) ─────────────────────────────────────────── */}
      <section style={{ background:DK, borderTop:'1px solid rgba(237,213,246,0.12)', padding:`${sectionPy} ${px}`, textAlign:'center' }}>
        <FadeIn style={{ marginBottom:48 }}>
          <span style={label}>Período de lanzamiento</span>
          <h2 style={{ ...h2style, marginBottom:'1rem' }}>
            <span style={{ display:'block', color:LT }}>Tres meses pro.</span>
            <span style={{ display:'block', color:PK }}>Cero pesos.</span>
            <span style={{ display:'block', color:LT, fontSize:'clamp(1.1rem,2.5vw,1.6rem)', opacity:0.75 }}>Solo hasta el 31 de agosto.</span>
          </h2>
          <p style={{ fontFamily:FN, fontSize:'0.95rem', color:'rgba(255,255,255,0.65)', maxWidth:520, margin:'0 auto 0', lineHeight:1.7 }}>
            Todos los comercios que se registren antes del 31 de agosto acceden al plan Pro sin costo. Sin tarjeta, sin letra chica.
          </p>
        </FadeIn>
        <div style={{ display:'grid', gridTemplateColumns: mobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap:16, maxWidth:800, margin:'0 auto 48px' }}>
          {FOUNDER_BENEFITS.map((b, i) => (
            <FadeIn key={i} delay={i * 70}>
              <div style={{ background:'rgba(237,213,246,0.07)', border:'1px solid rgba(237,213,246,0.14)', borderRadius:14, padding:'22px 16px', display:'flex', flexDirection:'column', alignItems:'center', gap:10 }}>
                <div style={{ color:LT }}>{b.icon}</div>
                <span style={{ fontFamily:FN, fontWeight:600, fontSize:'0.85rem', color:WH, lineHeight:1.3 }}>{b.label}</span>
              </div>
            </FadeIn>
          ))}
        </div>
        <FadeIn>
          <button onClick={onLoginClick} style={btnDark}>Quiero ser comercio fundador</button>
          <p style={{ fontFamily:FN, fontSize:'0.78rem', color:'rgba(237,213,246,0.35)', marginTop:'1rem', marginBottom:0 }}>
      ...
      <section id="planes" style={{ background:LT, padding:`${sectionPy} ${px}` }}>
        <FadeIn style={{ textAlign:'center', marginBottom:48 }}>
          <div style={{ display:'flex', justifyContent:'center', marginBottom:20 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:7, background:DK, color:LT, padding:'7px 16px', borderRadius:100, fontSize:'0.8rem', fontWeight:600, fontFamily:FN }}>
              <IconCrown size={13} stroke={2.5} color={LT} />
              Período fundador · Plan Pro gratis hasta el 31/08
            </div>
          </div>
          <span style={label}>Planes</span>
        ...
        <div style={{ display:'grid', gridTemplateColumns: mobile ? '1fr' : 'repeat(3, 1fr)', gap:20, maxWidth:960, margin:'0 auto' }}>
          {PLANS.map((plan, i) => (
            <FadeIn key={plan.name} delay={i * 80}
              ...
              {plan.popular && (
                <div style={{ position:'absolute', top:-14, left:'50%', transform:'translateX(-50%)', background:DK, color:LT, fontSize:'0.72rem', fontWeight:700, letterSpacing:'2px', textTransform:'uppercase', padding:'5px 14px', borderRadius:100, zIndex:1 }}>
                  Más popular
                </div>
              )}

              {/* Banner período fundador */}
              <div style={{ background:DK, borderRadius:'18px 18px 0 0', padding:'9px 18px', textAlign:'center', fontSize:'0.71rem', fontWeight:600, fontFamily:FN, color:LT, lineHeight:1.4 }}>
                {plan.name === 'Pro'
                  ? <>Estás en este plan · <span style={{ color:PK }}>hasta el 31/08</span></>
                  : <>Durante el período fundador incluye todo el plan Pro · <span style={{ color:PK }}>hasta el 31/08</span></>
                }
              </div>

              {/* Contenido de la card */}
              <div style={{ padding:'24px 28px 32px', display:'flex', flexDirection:'column', flex:1 }}>
                {/* Nombre y precio */}
                ...
                <div style={{ borderTop:'1px solid rgba(34,0,51,0.1)', marginTop:20, marginBottom:16, flex:1 }}>
                  {plan.permanentFeatures.map(feat => (
                    <div key={feat} style={{ padding:'8px 0', borderBottom:'1px solid rgba(34,0,51,0.06)', display:'flex', alignItems:'center', gap:10 }}>
                      <IconCheck size={16} stroke={2.5} color='#6F30DF' />
                      <span style={{ fontSize:'0.88rem', color:DK, fontFamily:FN }}>{feat}</span>
                    </div>
                  ))}
                </div>

                {/* Bloque fundador (solo Free y Starter) */}
                {plan.founderFeatures.length > 0 && (
                  <div style={{ background:'#f8f5ff', border:'1.5px dashed rgba(255,25,159,0.25)', borderRadius:'14px', padding:'1rem', marginBottom:'1.25rem' }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'0.75rem', flexWrap:'wrap', gap:6 }}>
                      <span style={{ fontSize:'0.68rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'1.5px', color:PK, fontFamily:FN }}>
                        Incluido hasta el 31/08
                      </span>
                      <span style={{ background:PK, color:'#fff', fontSize:'0.6rem', fontWeight:700, borderRadius:'100px', padding:'3px 9px', fontFamily:FN, whiteSpace:'nowrap' }}>
                        GRATIS HASTA EL 31/08
                      </span>
                    </div>
                    {plan.founderFeatures.map((feat, fi) => (
                      <div key={feat} style={{ padding:'6px 0', borderBottom: fi < plan.founderFeatures.length - 1 ? '1px solid rgba(255,25,159,0.10)' : 'none', display:'flex', alignItems:'center', gap:10 }}>
                        <IconCheck size={15} stroke={2.5} color={PK} />
                        <span style={{ fontSize:'0.85rem', color:'rgba(34,0,51,0.75)', fontFamily:FN }}>{feat}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {/* CTA */}
                {plan.name === 'Pro'
                  ? <button onClick={onLoginClick} style={{ background:LT, color:DK, width:'100%', textAlign:'center', padding:'0.9rem 2.25rem', borderRadius:100, fontWeight:700, fontSize:'0.95rem', border:'none', cursor:'pointer', fontFamily:FN }}>
                      Tu plan actual
                    </button>
                  : <button onClick={onLoginClick} style={{ ...btnLight, width:'100%', textAlign:'center' }}>
                      Empezar
                    </button>
                }
              </div>
      ...
      {/* ── CTA final (oscuro) ──────────────────────────────────────────── */}
      <section style={{ background:DK, minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'clamp(4rem,10vw,7rem) 2rem', textAlign:'center' }}>
        <FadeIn>
          <h2 style={{ fontFamily:FL, fontWeight:900, textTransform:'uppercase', fontSize:'clamp(2.5rem,7vw,4rem)', color:LT, letterSpacing:'-1px', margin:0, marginBottom:'1rem' }}>
            TU CLUB TE ESPERA.
          </h2>
          <p style={{ fontFamily:FN, fontWeight:400, fontSize:'1rem', color:'rgba(237,213,246,0.65)', margin:0, marginBottom:'2.5rem' }}>
            Empezá gratis hoy. Sin tarjeta, sin hardware, sin complicaciones.
          </p>
          <button onClick={onLoginClick} style={{ background:LT, color:DK, padding:'0.9rem 2.25rem', borderRadius:'100px', fontWeight:700, fontSize:'0.95rem', border:'none', cursor:'pointer', fontFamily:FN }}>
...
const FEATURES = [
  ...
  { icon:<IconBroadcast  size={28} stroke={1.5} color={DK} />, title:'WhatsApp semi-automático',      sub:'Clufix detecta el momento y prepara el mensaje — vos lo enviás con un tap.' },
...
const FOUNDER_BENEFITS = [
  { icon:<IconCrown       size={24} stroke={1.5} />, label:'Clientes ilimitados'     },
  { icon:<IconPercentage  size={24} stroke={1.5} />, label:'Cupones de descuento'    },
  { icon:<IconMessages    size={24} stroke={1.5} />, label:'WhatsApp semi-automático'     },
  { icon:<IconFlame       size={24} stroke={1.5} />, label:'Días con puntos dobles'  },
]
