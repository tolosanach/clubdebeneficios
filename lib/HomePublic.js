...
export default function HomePublic({ onLoginClick }) {
  ...
  const renderStep = (step, globalIdx, isDark) => {
    const active = activeSteps[globalIdx]
    const inactiveColor = isDark ? 'rgba(255,255,255,0.18)' : 'rgba(0,0,0,0.12)'
    const dotColor = active ? PK : inactiveColor
    // Top line lights up based on the previous step's activation (not the current step)
    const topLineColor = (globalIdx === 0 || globalIdx === 3)
      ? inactiveColor
      : (activeSteps[globalIdx - 1] ? PK : inactiveColor)
    const stepPad = mobile ? '16px' : '24px'
    const hPad = mobile ? '24px' : '10%'
    return (
      <FadeIn key={step.num}>
        <div
          ref={el => { stepRefs.current[globalIdx] = el }}
          style={{ display:'grid', gridTemplateColumns: mobile ? '40px 1fr' : '40px 1fr 1fr', maxWidth:1100, margin:'0 auto', padding:`${stepPad} ${hPad}`, columnGap:32, rowGap:40, alignItems:'stretch' }}
        >
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
            <div style={{ flex:1, width:2, background:topLineColor, transition:'background 0.4s ease' }} />
            <div style={{ width:20, height:20, borderRadius:'50%', flexShrink:0, background:dotColor, transition:'background 0.4s ease' }} />
            <div style={{ flex:1 }} />
          </div>
          <div style={{ alignSelf:'center', opacity: active ? 1 : 0.35, transform: active ? 'translateX(4px)' : 'translateX(0)', transition:'opacity 0.4s ease, transform 0.4s ease' }}>
            <div style={{ fontSize:'0.78rem', fontWeight:700, letterSpacing:'3px', color: isDark ? 'rgba(255,255,255,0.35)' : 'rgba(34,0,51,0.35)', marginBottom:8, textTransform:'uppercase' }}>{step.num}</div>
            <h2 style={{ ...h2style, color: isDark ? WH : DK, marginBottom:12 }}>{step.title}</h2>
            <p style={{ margin:0, lineHeight:1.7, color: isDark ? 'rgba(255,255,255,0.70)' : 'rgba(34,0,51,0.65)', maxWidth:440 }}>{step.desc}</p>
          </div>
          {!mobile && (
            <div style={{ alignSelf:'center', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <div style={{ width:180, height:180, borderRadius:24, background: isDark ? 'rgba(237,213,246,0.08)' : 'rgba(34,0,51,0.08)', display:'flex', alignItems:'center', justifyContent:'center' }}>
  ...
  return (
    <div style={{ fontFamily:FN, overflowX:'hidden', isolation:'isolate' }}>
      ...
      {/* ── COMERCIO block ──────────────────────────────────────────────── */}
      <section id="como-funciona" style={{ background:DK }}>
        <FadeIn>
          <div style={{ maxWidth:1100, margin:'0 auto', padding: mobile ? '48px 24px 0' : '64px 10% 0' }}>
            <div style={{ display:'grid', gridTemplateColumns: mobile ? '40px 1fr' : '40px 1fr 1fr', columnGap:32 }}>
              <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'center' }}>
                <div style={{ width:2, height:40, background:'rgba(255,255,255,0.18)' }} />
              </div>
              <div>
                <span style={label}>COMERCIO</span>
                <h2 style={{ ...h2style, color:WH, fontWeight:900 }}>VOS LO ARMÁS.</h2>
                <p style={{ marginTop:20, color:'rgba(255,255,255,0.70)', lineHeight:1.7, maxWidth:440 }}>
                  Tu club listo en 2 minutos. Tus reglas, tus premios, tu forma.
                </p>
              </div>
            </div>
          </div>
        </FadeIn>
        {STEPS_COMMERCE.map((step, i) => renderStep(step, i, true))}
      </section>

      {/* ── Divisor ─────────────────────────────────────────────────────── */}
      <div style={{ background:WH, padding:'20px', textAlign:'center' }}>
        <span style={{ fontSize:'0.72rem', fontWeight:700, letterSpacing:'4px', textTransform:'uppercase', color:'rgba(34,0,51,0.35)' }}>Y ENTONCES...</span>
      </div>

      {/* ── CLIENTE block ───────────────────────────────────────────────── */}
      <section style={{ background:LT }}>
        <FadeIn>
          <div style={{ maxWidth:1100, margin:'0 auto', padding: mobile ? '48px 24px 0' : '64px 10% 0' }}>
            <div style={{ display:'grid', gridTemplateColumns: mobile ? '40px 1fr' : '40px 1fr 1fr', columnGap:32 }}>
              <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'center' }}>
                <div style={{ width:2, height:40, background:'rgba(0,0,0,0.12)' }} />
              </div>
              <div>
                <span style={label}>CLIENTE</span>
                <h2 style={{ ...h2style, color:DK, fontWeight:900 }}>VOS LO DISFRUTÁS.</h2>
                <p style={{ marginTop:20, color:'rgba(34,0,51,0.65)', lineHeight:1.7, maxWidth:440 }}>
                  Escaneá, acumulá y canjeá. Sin descargar nada.
                </p>
              </div>
            </div>
          </div>
        </FadeIn>
        {STEPS_CLIENT.map((step, i) => renderStep(step, i + 3, false))}
      </section>
...
const STEPS_CLIENT = [
  ...
  { num:'06', tag:'El cliente',  title:'CANJEA SU PREMIO',        desc:'Cuando llega al tope, el sistema lo avisa. Elige su premio, vos confirmás con un escaneo. Simple y sin papel.',                                                                                                                                                        light:false, visual:'🎁' },
]
