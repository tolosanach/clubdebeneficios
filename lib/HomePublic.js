...
const btnGhost = { background:'transparent', color:'rgba(255,255,255,0.8)', padding:'0.9rem 2.25rem', borderRadius:100, fontWeight:500, border:'1.5px solid rgba(255,255,255,0.2)', cursor:'pointer', fontFamily:FN, display:'inline-block', textDecoration:'none' }
...
const h2style  = { fontFamily:FM, fontSize:'clamp(1.8rem, 4vw, 2.4rem)', fontWeight:900, letterSpacing:'0px', textTransform:'uppercase', lineHeight:1.1, margin:0 }
...
function Nav({ onLoginClick }) {
  return (
    <nav style={{ height:68, background:'rgba(15,8,32,0.96)', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', alignItems:'center', padding:'0 clamp(16px, 5vw, 48px)', gap:32, fontFamily:FN }}>
      <a href="/" style={{ flexShrink:0, textDecoration:'none' }}>
        <Logo height={44} />
      </a>
      <div style={{ flex:1, display:'flex', alignItems:'center', gap:28 }}>
        <a href="#planes" style={{ color:'rgba(255,255,255,0.72)', fontSize:'0.9rem', fontWeight:500, textDecoration:'none' }}>Precios</a>
        <a href="#rubros" style={{ color:'rgba(255,255,255,0.72)', fontSize:'0.9rem', fontWeight:500, textDecoration:'none' }}>Rubros</a>
      </div>
      <button onClick={onLoginClick} style={{ background:PK, color:'#fff', padding:'0.5rem 1.25rem', borderRadius:100, border:'none', cursor:'pointer', fontFamily:FN, fontWeight:700, fontSize:'0.9rem' }}>
        Empezar gratis
      </button>
    </nav>
  )
...
// ─── Rubro icons map ──────────────────────────────────────────────────────────
const RUBRO_ICONS = {
  peluquerias:  <IconScissors size={32} stroke={1.5} color="#fff" />,
  farmacias:    <IconPill size={32} stroke={1.5} color="#fff" />,
  indumentaria: <IconHanger size={32} stroke={1.5} color="#fff" />,
  restaurantes: <IconToolsKitchen2 size={32} stroke={1.5} color="#fff" />,
  gimnasios:    <IconBarbell size={32} stroke={1.5} color="#fff" />,
  jugeterias:   <IconBuildingStore size={32} stroke={1.5} color="#fff" />,
}
    ...
    <div style={{ fontFamily:FN, overflowX:'hidden' }}>
      ...
      <section style={{ background:V1, padding:`80px ${px}`, textAlign:'center', color:'#fff' }}>
        <FadeIn>
          ...
          <h1 style={{ fontFamily:FM, fontSize:'clamp(2.8rem,8vw,5rem)', fontWeight:900, letterSpacing:'0px', textTransform:'uppercase', lineHeight:1.05, margin:'0 0 24px', maxWidth:700, marginLeft:'auto', marginRight:'auto' }}>
            Tus clientes ya existen.<br />
          ...
          <div style={{ display:'flex', gap:14, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={onLoginClick} style={btnPk}>Crear mi club gratis</button>
        ...
        <div style={{ display:'grid', gridTemplateColumns: mobile ? '1fr' : 'repeat(3, 1fr)', gap:16, maxWidth:1100, margin:'0 auto' }}>
          {BENTO.map((card, i) => (
            <FadeIn key={i} delay={i * 60}
              ...
              {card.big && <div style={{ fontSize:'3.2rem', fontWeight:900, letterSpacing:'-3px', lineHeight:1, color: card.light ? V3 : '#fff' }}>{card.big}</div>}
              <div>
              ...
              <div style={{ borderTop:'1px solid #f0eaff', margin:'20px 0', flex:1 }}>
                {plan.features.map(f => (
                  <div key={f.label} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid #f9f6ff' }}>
                    {f.ok
                      ? <IconCheck size={16} stroke={2.5} color={V3} />
                      : <IconX     size={16} stroke={2.5} color='rgba(26,0,80,0.25)' />
                    }
                    <span style={{ fontSize:'0.88rem', color: f.ok ? V1 : 'rgba(26,0,80,0.4)', fontFamily:FN }}>{f.label}</span>
                  </div>
                ))}
              </div>
              <button onClick={onLoginClick} style={{ ...btnPk, width:'100%', textAlign:'center', marginTop:8 }}>
                Empezar
              </button>
            </FadeIn>
          ))}
          ...
          {TESTIMONIOS.map((t, i) => (
            <FadeIn key={i} delay={i * 80}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:38, height:38, borderRadius:'50%', background:PK, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, color:'#fff', fontSize:'0.9rem', fontFamily:FN, flexShrink:0 }}>
                  {t.initials}
                </div>
                <div>
...
const BENTO = [
  ...
  { bg:V4,  light:true,  icon:<IconUsersGroup size={28} stroke={1.5} color={V3} />, title:'Segmentación automática', sub:'Nuevos, frecuentes, VIP e inactivos.' },
  { bg:V4,  light:true,  icon:<IconBroadcast  size={28} stroke={1.5} color={V3} />, title:'WhatsApp automático',      sub:'Mensajes de reactivación sin trabajo extra.' },
  { bg:V2,  light:false, wide:true, icon:<IconMapPin size={28} stroke={1.5} color={'rgba(255,255,255,0.7)'} />, title:'Directorio público de Clufix', sub:'Tu club aparece en el directorio para sumar nuevos socios.' },
  { bg:V4,  light:true,  icon:<IconBell   size={28} stroke={1.5} color={V3} />, title:'Notificaciones push', sub:'Tus clientes se enteran de todo.' },
  { bg:V1,  light:false, big:'$0',     title:'Para empezar hoy',  sub:'Plan Free para siempre, sin tarjeta.' },
  { bg:V4,  light:true,  icon:<IconRobot  size={28} stroke={1.5} color={V3} />, title:'Soporte con IA', sub:'Disponible 24/7 dentro de la app.' },
]
