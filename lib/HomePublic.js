...
function Nav({ onLoginClick }) {
  return (
    <nav style={{ height:80, background:DK, borderBottom:`1px solid rgba(237,213,246,0.12)`, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 clamp(16px,5vw,48px)', fontFamily:FN }}>
...
export default function HomePublic({ onLoginClick }) {
  ...
  return (
    <div style={{ fontFamily:FN, overflowX:'hidden' }}>
      ...
      {/* ── Hero (oscuro) ───────────────────────────────────────────────── */}
      <section style={{ background:DK, padding:`80px ${px}`, textAlign:'center' }}>
        <FadeIn>
      ...
      {/* ── Split intro: izquierda oscura / derecha clara ────────────────── */}
      <section style={{ display:'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr' }}>
        <FadeIn style={{ background:DK, padding:`${sectionPy} ${px}` }}>
          <span style={label}>Para el comercio</span>
          <h2 style={{ ...h2style, color:WH }}>Vos ponés las reglas.</h2>
        ...
        <FadeIn style={{ background:LT, padding:`${sectionPy} ${px}` }} delay={100}>
          <span style={label}>Para el cliente</span>
          <h2 style={{ ...h2style, color:DK }}>Ellos acumulan.</h2>
      ...
      <section id="como-funciona">
        ...
        <div style={{ display:'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr' }}>
          <div style={{ background:DK, padding:`48px ${px}` }}>
            <p style={{ fontWeight:700, fontSize:'1.1rem', margin:'0 0 8px', color:WH }}>Club configurado.</p>
            <p style={{ color:'rgba(255,255,255,0.55)', margin:0 }}>El QR está en el mostrador.</p>
          </div>
          <div style={{ background:LT, padding:`48px ${px}` }}>
            <p style={{ fontWeight:700, fontSize:'1.1rem', margin:'0 0 8px', color:DK }}>Llega el cliente.</p>
            <p style={{ color:'rgba(34,0,51,0.55)', margin:0 }}>Escanea el QR y empieza.</p>
          </div>
        </div>
      </section>

      {/* ── Bento features (fondo blanco, cards alternadas) ─────────────── */}
          ...
          {BENTO.map((card, i) => (
            <FadeIn key={i} delay={i * 60}
              style={{
                ...
                background: card.dark ? DK : LT,
                borderRadius:20, padding:'32px 28px',
                display:'flex', flexDirection:'column', justifyContent:'space-between', gap:16, minHeight:160,
              }}>
              {card.icon && <div style={{ opacity:0.9 }}>{card.icon}</div>}
              {card.big && <div style={{ fontSize:'3.2rem', fontWeight:900, letterSpacing:'-3px', lineHeight:1, color: card.dark ? PK : DK }}>{card.big}</div>}
              <div>
                <p style={{ margin:0, fontWeight:700, fontSize:'1rem', color: card.dark ? WH : DK }}>{card.title}</p>
                {card.sub && <p style={{ margin:'6px 0 0', fontSize:'0.85rem', color: card.dark ? 'rgba(255,255,255,0.6)' : 'rgba(34,0,51,0.55)' }}>{card.sub}</p>}
              </div>
            </FadeIn>
          ))}
        </div>
...
// ─── Static data ──────────────────────────────────────────────────────────────
const STEPS = [
  { num:'01', tag:'El comercio', title:'Te registrás con Google',  desc:'En 2 minutos tenés tu cuenta lista. Solo necesitás tu email de Google — sin formularios largos.',         light:false, visual:'🔑' },
  { num:'02', tag:'El comercio', title:'Configurás tu club',        desc:'Elegís el nombre, el sistema (estrellas o puntos), los premios y cuánto vale cada visita.',               light:true,  visual:'⚙️' },
  { num:'03', tag:'El comercio', title:'Tu kit físico está listo',  desc:'Imprimís el QR de tu local y lo ponés en el mostrador. Eso es todo lo que necesitás.',                    light:false, visual:'📌' },
  { num:'04', tag:'El cliente',  title:'Escanea y se suma',         desc:'El cliente apunta la cámara al QR, se registra en segundos y recibe un cupón de bienvenida automático.',  light:true,  visual:'📱' },
  { num:'05', tag:'El cliente',  title:'Acumula recompensas',       desc:'Cada visita suma. El cliente ve su progreso en tiempo real y tiene una razón concreta para volver.',     light:false, visual:'⭐' },
  { num:'06', tag:'El cliente',  title:'Canjea su premio',          desc:'Cuando llega al tope, el sistema lo avisa y el cliente elige su premio. El stock se descuenta solo.',    light:true,  visual:'🎁' },
]

const BENTO = [
  { dark:true,  big:'30"',   title:'Para registrar una visita',    sub:'Escaneás el QR del cliente y listo.' },
  { dark:false, icon:<IconUsersGroup size={28} stroke={1.5} color={DK} />, title:'Segmentación automática', sub:'Nuevos, frecuentes, VIP e inactivos.' },
  { dark:true,  icon:<IconBroadcast  size={28} stroke={1.5} color={LT} />, title:'WhatsApp automático',      sub:'Mensajes de reactivación sin trabajo extra.' },
  { dark:false, wide:true, icon:<IconMapPin size={28} stroke={1.5} color={DK} />, title:'Directorio público de Clufix', sub:'Tu club aparece en el directorio para sumar nuevos socios.' },
  { dark:true,  icon:<IconBell  size={28} stroke={1.5} color={LT} />, title:'Notificaciones push',          sub:'Tus clientes se enteran de todo.' },
  { dark:false, big:'$0',    title:'Para empezar hoy',              sub:'Plan Free para siempre, sin tarjeta.' },
  { dark:true,  icon:<IconRobot size={28} stroke={1.5} color={LT} />, title:'Soporte con IA',               sub:'Disponible 24/7 dentro de la app.' },
]
