...
export default function HomePublic({ onLoginClick }) {
  ...
  const renderStep = (step, globalIdx) => {
    const dark = !step.light
    const active = activeSteps[globalIdx]
    const dotColor = active ? PK : (dark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)')
    const lineColor = active ? PK : (dark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)')
    const showTopLine = globalIdx > 0 && globalIdx !== 3
    return (
      <FadeIn key={step.num} style={{ background: dark ? DK : LT }}>
        <div
          ref={el => { stepRefs.current[globalIdx] = el }}
          style={{ display:'grid', gridTemplateColumns: mobile ? '40px 1fr' : '40px 1fr 1fr', maxWidth:1100, margin:'0 auto', padding:`${sectionPy} ${px}`, columnGap:32, rowGap:40, alignItems:'stretch' }}
        >
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
            <div style={{ flex:1, width:2, background: showTopLine ? lineColor : 'transparent', transition:'background 0.4s ease' }} />
          ...
          <div style={{ alignSelf:'center', opacity: active ? 1 : 0.35, transform: active ? 'translateX(4px)' : 'translateX(0)', transition:'opacity 0.4s ease, transform 0.4s ease' }}>
            ...
            <div style={{ fontSize:'0.78rem', fontWeight:700, letterSpacing:'3px', color: dark ? 'rgba(255,255,255,0.35)' : 'rgba(34,0,51,0.35)', marginBottom:12, textTransform:'uppercase' }}>{step.num}</div>
            <h2 style={{ ...h2style, color: dark ? WH : DK }}>{step.title}</h2>
            <p style={{ marginTop:16, lineHeight:1.7, color: dark ? 'rgba(255,255,255,0.70)' : 'rgba(34,0,51,0.65)', maxWidth:440 }}>{step.desc}</p>
          </div>
          {!mobile && (
  ...
  return (
    <div style={{ fontFamily:FN, overflowX:'hidden', isolation:'isolate' }}>
      ...
      <section id="como-funciona">
        {STEPS_COMMERCE.map((step, i) => renderStep(step, i))}
        ...
        {STEPS_CLIENT.map((step, i) => renderStep(step, i + 3))}
      </section>
