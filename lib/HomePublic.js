...
export default function HomePublic({ onLoginClick }) {
  ...
  const renderStep = (step, globalIdx, isDark) => {
    ...
    const dotColor = active ? PK : (isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)')
    const lineColor = active ? PK : (isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)')
      ...
      <FadeIn key={step.num}>
        ...
        >
          ...
          <div style={{ alignSelf:'center', opacity: active ? 1 : 0.35, transform: active ? 'translateX(4px)' : 'translateX(0)', transition:'opacity 0.4s ease, transform 0.4s ease' }}>
            <div style={{ fontSize:'0.78rem', fontWeight:700, letterSpacing:'3px', color: isDark ? 'rgba(255,255,255,0.35)' : 'rgba(34,0,51,0.35)', marginBottom:12, textTransform:'uppercase' }}>{step.num}</div>
            <h2 style={{ ...h2style, color: isDark ? WH : DK }}>{step.title}</h2>
            <p style={{ marginTop:16, lineHeight:1.7, color: isDark ? 'rgba(255,255,255,0.70)' : 'rgba(34,0,51,0.65)', maxWidth:440 }}>{step.desc}</p>
          </div>
          {!mobile && (
  ...
  return (
    <div style={{ fontFamily:FN, overflowX:'hidden', isolation:'isolate' }}>
      ...
      <section id="como-funciona" style={{ background:DK }}>
        <FadeIn>
          <div style={{ maxWidth:1100, margin:'0 auto', padding:`${sectionPy} ${px} 40px` }}>
