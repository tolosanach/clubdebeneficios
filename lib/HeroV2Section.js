...
// Frases rotativas del headline ("De 'hola' a 'X'"). Cada 2.6s la línea
// inferior cambia de frase con un slide-up animado. El apóstrofe de cierre
// alterna color entre violeta (#BD4BF8) y naranja (#FE5000) según el index.
const HERO_PHRASES = [
  'tu primer cliente',
  'puntos acumulados',
  'premio canjeado',
...
// Frases 1, 3, 5, 7 (index 0, 2, 4, 6) → violeta; frases 2, 4, 6 (index
// 1, 3, 5) → naranja. La paridad del índice define el color.
const apostropheColorFor = (i) => i % 2 === 0 ? '#BD4BF8' : '#FE5000'
...
export default function HeroV2Section({ setView, user, profile, onLogin }) {
  ...
  // Loop de frases del headline — cada 2.6s avanza al siguiente index.
  // El span interno usa key={phraseIndex} para forzar remount y reiniciar
  // la animation 'hero-v2-phrase-cycle' (entra desde abajo → reposo →
  // sale por arriba). Cleanup borra el interval al desmontar.
  useEffect(() => {
  ...
  return (
    <section ref={sectionRef} style={{
      ...
      <style>{`
        ...
        /* Ciclo de frase del headline — slide up continuo en 2600ms.
           Entra desde abajo (translateY 100%, opacity 0) en los primeros
           450ms (~17.3% del total), se queda visible hasta los últimos
           450ms, y sale por arriba (translateY -100%, opacity 0). El
           remount via key={phraseIndex} sincroniza con el setInterval
           de 2600ms para que el ciclo sea continuo. */
        @keyframes hero-v2-phrase-cycle {
          0%     { transform: translateY(100%); opacity: 0; }
          17.3%  { transform: translateY(0);    opacity: 1; }
          82.7%  { transform: translateY(0);    opacity: 1; }
      ...
      <div style={{
        ...
        {/* Capa 1 — Tinte violeta brand (rebrand mayo 2026, fase 2).
            Antes era gradient naranja→fucsia→violeta; ahora es un
            gradient violeta-monocromo (deep → brand → light) que
            mantiene la sensacion de profundidad sin contradecir la
            nueva identidad solida. mixBlendMode 'overlay' tinta el
            video sin aplastarlo. */}
        <div style={{
          position: 'absolute', inset: 0,
          background: 'linear-gradient(135deg, rgba(105,53,189,0.42) 0%, rgba(113,49,225,0.42) 50%, rgba(167,139,250,0.42) 100%)',
          mixBlendMode: 'overlay',
          pointerEvents: 'none',
        }} />

        {/* Capa 2 — Bloom radial monocromo violeta (rebrand mayo 2026, fase 2).
            Antes 3 halos naranja+violeta+fucsia. Ahora 3 halos del
            mismo violeta brand a distintas profundidades (deep, brand,
            light) — preserva el movimiento organico de "liquido
            coloreado" pero sin rosa ni naranja. mixBlendMode 'screen'
            suma luz. */}
        <div style={{
          position: 'absolute', inset: '-10%',
          background: 'radial-gradient(circle at 28% 42%, rgba(105,53,189,0.40), transparent 55%), radial-gradient(circle at 72% 60%, rgba(113,49,225,0.38), transparent 52%), radial-gradient(circle at 50% 50%, rgba(167,139,250,0.22), transparent 60%)',
          mixBlendMode: 'screen',
      ...
      <div style={{
        ...
        {/* Headline animado — 2 líneas. La primera estática "De 'hola' a"
            y la segunda con una frase rotativa entre apóstrofes que
            cambia cada 2.6s con slide up (entra desde abajo, sale por
            arriba). El apóstrofe de cierre alterna color violeta/naranja
            según la frase. El wrapper exterior aplica el fade-in inicial;
            el span interno tiene su propia animation cíclica. */}
        <h1 style={{
          fontFamily: FN,
          ...
          <span style={{ display: 'block' }}>De &apos;hola&apos; a</span>
          <span style={{
            display: 'block',
            height: '1.1em',
            lineHeight: 1.05,
            clipPath: 'inset(0 -999px 0 -999px)',
          }}>
            <span
              key={phraseIndex}
              style={{
                ...
                whiteSpace: 'nowrap',
                animation: 'hero-v2-phrase-cycle 2600ms cubic-bezier(0.4, 0, 0.2, 1) both',
                willChange: 'transform, opacity',
              }}
            >
              &apos;{HERO_PHRASES[phraseIndex]}<span style={{ color: apostropheColorFor(phraseIndex) }}>&apos;</span>
            </span>
          </span>
        </h1>
