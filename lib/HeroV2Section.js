...
//   - Navbar global de Clufix se mantiene (no la duplicamos).
...
export default function HeroV2Section({ setView, user, profile, onLogin }) {
  ...
  const twRef = useRef({ phraseIdx: 0, charIdx: 0, phase: 'typing' })

  useEffect(() => {
    let timer
    function tick() {
      const tw = twRef.current
      const phrase = HERO_PHRASES[tw.phraseIdx]

      if (tw.phase === 'typing') {
        if (tw.charIdx < phrase.length) {
          tw.charIdx++
          setDisplayed(phrase.slice(0, tw.charIdx))
          setShowCursor(true)
          timer = setTimeout(tick, 35 + Math.random() * 50)
        } else {
          tw.phase = 'pause-after'
          setShowCursor(false)
          timer = setTimeout(tick, 2000)
        }
      } else if (tw.phase === 'pause-after') {
        tw.phase = 'deleting'
        setShowCursor(true)
        tick()
        ...
        if (tw.charIdx > 0) {
          ...
          tw.phase = 'pause-before'
          timer = setTimeout(tick, 400)
        }
      } else if (tw.phase === 'pause-before') {
        tw.phraseIdx = (tw.phraseIdx + 1) % HERO_PHRASES.length
        tw.charIdx = 0
        tw.phase = 'typing'
        tick()
      }
    ...
    <section ref={sectionRef} style={{
      ...
      <div style={{
        ...
        <h1 style={{
          ...
          <span style={{ display: 'block' }}>Un QR,</span>
          <span style={{ display: 'block', minHeight: '1.1em' }}>
            {displayed}<span style={showCursor ? { animation: 'tw-cursor-blink 530ms step-end infinite' } : { opacity: 0 }}>|</span>
          </span>
        </h1>
        ...
        <p style={{
          ...
          Sumás puntos en cada visita y los canjeás por premios reales.
        </p>
