...
export default function SuggestionsInbox() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('benefix:support-chat-open', onChatOpen)
    return () => window.removeEventListener('benefix:support-chat-open', onChatOpen)
  }, [])

  // Cuando se abre el inbox, avisamos al chat para que se cierre
  useEffect(() => {
    if (open) window.dispatchEvent(new CustomEvent('benefix:inbox-open'))
  }, [open])
  ...
  function handleCta(s, e) {
    ...
    if (s.cta_url.startsWith('#')) {
      ...
      if (s.target === 'merchant') {
        try { localStorage.setItem('benefix:commerceTab', tab) } catch {}
      }
    ...
    <>
      ...
      {!open && (
        <button onClick={() => setOpen(true)} aria-label="Abrir buzón de sugerencias"
          style={{
            position:'fixed', bottom:158, right:22, zIndex:9997,
