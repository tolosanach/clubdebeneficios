...
// Etiqueta de %OFF con el número cambiando entre 10/15/20/30 cíclicamente.
// El badge tiene un pulse violeta sutil para llamar la atención.
function DiscountAnim({ active }) {
  ...
  return (
    <span style={{
      ...
      background: active ? 'linear-gradient(135deg, #FE5000, #BD4BF8)' : 'rgba(255,255,255,0.08)',
      borderRadius: 5,
      padding: '2px 6px',
      transition: 'background 220ms ease',
      boxShadow: active ? '0 2px 8px rgba(189,75,248,0.40)' : 'none',
    }}>
      <span style={{
 ...
 */
export default function RecompensasTabAnim({ tabId, systemType = 'stars', active = true }) {
  let anim = null
  let label = ''
  if (tabId === 'how') {
    ...
    anim = <DiscountAnim active={active} />
    label = 'Descuento'
  }
  return (
    <span style={{
