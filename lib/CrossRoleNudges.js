...
// CrossRoleNudges — modal temporizado que aparece para sugerir al cliente
// que registre un negocio (BizSignupNudge, 10s). Solo se muestra una vez
// por dispositivo (flag en localStorage). Respeta el flag global
// `clufix:bizAnswer` para no nag-ear si el user ya respondió desde el banner.

import { useEffect, useRef, useState } from 'react'
import { Store, X, Info, ArrowRight } from 'lucide-react'
...
const BIZ_ANSWER_KEY = 'clufix:bizAnswer'
const BIZ_NUDGE_KEY  = 'clufix:bizNudgeSeen'
    ...
    <ModalShell onClose={close}>
      <div style={{ textAlign: 'center', marginBottom: 18, marginTop: 4 }}>
        <div style={{
          ...
          <Info size={26} color="#6F30DF" strokeWidth={2} />
        </div>
...
export default function CrossRoleNudges({ profile }) {
  return <BizSignupNudge profile={profile} />
}
