...
import {
  ...
  RefreshCw, Ban, ExternalLink, Sparkles, Crown,
} from 'lucide-react'
...
const C = {
  ...
  // Rebrand mayo 2026 fase 2: v migra a violeta brand sólido #6F30DF.
...
// usaban violeta brand viejo (#6F30DF / #6F30DF). Migran a #6F30DF.
...
const TYPE_COLOR = {
  ...
  join:               '#22C55E',
  prize_redeem:       '#FF199F',
  discount_redeem:    '#FF199F',
  discount_renewed:   '#22C55E',
  discount_declined:  'rgba(255,255,255,0.55)',
  discount_granted:   '#6F30DF',
  no_prizes_warning:  '#6F30DF',
  founder_expired:    '#EDD5F6',
  new_prize:          '#FF199F',  // rosa fucsia — mismo que prize_redeem
  new_promo:          '#FF199F',  // naranja — mismo que discount_redeem
}
...
export default function NotificationsBell({ bottom = 156, role = 'client', hideButton = false, mode = 'drawer' }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('clufix:open-notifications', onOpen)
    return () => window.removeEventListener('clufix:open-notifications', onOpen)
  }, [])

  // Cerrar el drawer cuando el user navega a otra view/tab via
  // clufix:navigate. Sin esto, tocar Beneficios o cualquier otro slot
  ...
  useEffect(() => {
    ...
    window.addEventListener('clufix:navigate', onNav)
    return () => window.removeEventListener('clufix:navigate', onNav)
  }, [])
  ...
  useEffect(() => {
    window.dispatchEvent(new CustomEvent('clufix:notifications-count', { detail: { count: totalUnread } }))
  }, [totalUnread])
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('clufix:notifications-refresh', onRefresh)
    }
  }, [])
      ...
      try {
        ...
        channel = supabase
          ...
          .on('postgres_changes', {
            ...
            window.dispatchEvent(new CustomEvent('clufix:notifications-refresh'))
            setHasPulse(true)
  ...
  function handleSuggestionCta(s, e) {
    ...
    if (s.cta_url.startsWith('#')) {
      ...
      if (isMerchant) {
        try { localStorage.setItem('clufix:commerceTab', tabId) } catch {}
      }
      setOpen(false)
      ...
      {(open || isView) && (
        <>
          ...
          <aside style={isView ? {
            ...
            // Cliente/Comercio + el logo Clufix sigan accesibles cuando
                  ...
                  try {
                    ...
                    if (d.ok) {
                      alert('Push enviado. Revisa las notificaciones del sistema.\n\n' +
                        ...
                        '- En iOS: tenes que tener Clufix agregado a Pantalla de Inicio\n' +
