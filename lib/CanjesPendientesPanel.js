...
// 'benefix:notifications-refresh' para que la campana también se actualice.
...
export default function CanjesPendientesPanel({ commerce, unitIcon, unitLabel, onChange }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('benefix:notifications-refresh', onRefresh)
  }, [load])
  ...
  async function handleConfirm(item) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        ...
        // Sacar el item localmente — el dueño ya no lo ve en pendientes.
        setItems(arr => arr.filter(x => x.id !== item.id))
        onChange?.()
        try { window.dispatchEvent(new CustomEvent('benefix:notifications-refresh')) } catch {}
      }
    } catch (e) {
              ...
              <div key={item.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
                  ...
                  <div style={{
                    padding: '5px 11px',
                    borderRadius: 8,
                    background: 'rgba(189,75,248,0.14)',
                ...
                <div style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.65)', display: 'flex', alignItems: 'center', gap: 5 }}>
                      <UnitIcon size={11} {...(isStars ? { strokeWidth: 0, fill: '#BD4BF8' } : { strokeWidth: 2, color: '#BD4BF8' })} />
