  ...
  return (
    <>
      <div style={{
        ...
        <Star size={18} color="#7131E1" strokeWidth={2.2} style={{ flexShrink: 0, marginTop: 1 }} />
        ...
        {[500, 1000, 2000, 5000].map(amount => {
          ...
          return (
            <button
              ...
              {active && <Check size={11} color="#7131E1" strokeWidth={2.6} style={{ marginLeft: 5, verticalAlign: '-1px' }} />}
            </button>
          )
        })}
      </div>
...
function PresetCard({ active, onClick, title, desc }) {
  return (
    <button
      ...
      <div style={{
        width: 18, height: 18, borderRadius: '50%',
        border: `2px solid ${active ? '#7131E1' : 'rgba(255,255,255,0.25)'}`,
        background: active ? '#7131E1' : 'transparent',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        flexShrink: 0, marginTop: 2,
      }}>
        {active && <Check size={11} color="#fff" strokeWidth={3} />}
