...
const ProtectedRoute: React.FC<{ children: React.ReactNode, roles?: UserRole[] }> = ({ children, roles }) => {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center text-slate-400 font-black tracking-widest uppercase text-xs">Cargando...</div>;
  ...
  return (
    <Routes>
      ...
      <Route path="/" element={
        ...
        [UserRole.COMMERCE_OWNER, UserRole.STAFF_MANAGER, UserRole.VIEWER].includes(user.role) ? <Navigate to="/commerce" /> :
        user.role === UserRole.SCANNER ? <Navigate to="/commerce/scan" /> :
        <Navigate to="/customer" />
      } />
      <Route path="/admin/*" element={
        <ProtectedRoute roles={[UserRole.SUPER_ADMIN]}>
          <Layout title="Admin Central"><Routes><Route path="/" element={<SuperAdminDashboard />} /></Routes></Layout>
        </ProtectedRoute>
      } />
      <Route path="/commerce/*" element={
        <ProtectedRoute roles={[UserRole.COMMERCE_OWNER, UserRole.STAFF_MANAGER, UserRole.SCANNER, UserRole.VIEWER]}>
      ...
      <Route path="/customer/*" element={
        <ProtectedRoute roles={[UserRole.CUSTOMER]}>
          <Layout title="Club de Beneficios"><Routes><Route path="/" element={<CustomerPortal />} /><Route path="/benefits" element={<div className="p-8 bg-white rounded-3xl border">Explorar Beneficios</div>} /></Routes></Layout>
        </ProtectedRoute>
      } />
